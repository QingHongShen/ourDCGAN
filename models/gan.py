...
class GAN(object):
    def __init__(self, args):
        ...
        self.G = nn.Sequential(
            ...
            nn.Linear(512, 1024),
            nn.LeakyReLU(0.2),
            nn.Linear(1024, 32*32*3),  # 输出层修改为32x32x3
            nn.Tanh())
            ...
            for i, (images, _) in enumerate(train_loader):
                ...
                if ((i + 1) % 100) == 0:
                    ...
                    if self.cuda:
                        ...
                        z = Variable(torch.randn(self.batch_size, 100))
                    ###
                    #generated_images_32x32 = self.G(z)
                    generated_images_32x32 = self.G(z).mul(255).add_(0.5).byte()  # 归一化到0-255范围，并转换为byte类型
                    #generated_images_197x197 = F.interpolate(generated_images_32x32.view(self.batch_size, 3, 32, 32),
                    #                                         size=(197, 197), mode='bilinear', align_corners=False)
                    ###
                    ...
                    info = {
                        ...
                        #'generated_images': self.to_np(
                        #    generated_images_197x197.view(-1, 197, 197)[:self.number_of_images])
                    }
                ...
                if generator_iter % 1000 == 0:
                    ...
                    utils.save_image(grid, 'training_result_images/gan_image_iter_{}.png'.format(
                        str(generator_iter).zfill(3)), normalize=True )
